...
The cost-edit brainstorm's top idea (r85) was "fuzzy anchor recovery." But inspecting slivr's vendored
...
## The real cost lever
slivr's anchor protocol is cheap on INPUT (never re-sends whole files) but to replace a whole function
...
> Replace a whole function/class/method by NAME: the model sends only the **new** definition; slivr
...
- `src/agent.mjs`: registered in both tool maps (Session wraps it to capture the streaming diff) +
  system prompt. `src/repl.mjs` / `bin/slivr.mjs` / `src/ui.mjs`: approval preview, diff, step label.
...
`edit_symbol greet` and slivr applied `+2 -2 (symbol)` cleanly, sandboxed to the working directory.
